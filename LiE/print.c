...
(info_ind_tp* info,short ninfo,char* name,symblst arg,boolean doc)
{
  ...
  if (!arg && strcmp(name, "lie_index")==0)
...
/* change search path */
  ...
  { Printf("%s\n", result);
    if (foundinfo->directory) /* Print lie_index directory */
    {
      build_index(info,ninfo);
      if (doc) printindex();
    }
...
void printexpr(char* c, boolean pr, boolean doc)
{
  ...
  if (ninfo==0) ninfo=build_info_index(info); /* get lie_index */
